		...
		if (sw.pollSkeleton())
		{
			for( int ii = 0; ii < (int)Kinect.NuiSkeletonPositionIndex.Count; ii++) {
				...
				if( ((uint)Mask & (uint)(1 << ii) ) > 0 ){
					...
					//float delta = 0;
					//delta += (sw.bonePos[player,ii].x - _bones[ii].transform.localPosition.x/scale)*(sw.bonePos[player,ii].x - _bones[ii].transform.localPosition.x/scale);
					//delta += (sw.bonePos[player,ii].y - _bones[ii].transform.localPosition.y/scale)*(sw.bonePos[player,ii].y - _bones[ii].transform.localPosition.y/scale);
					//delta += (sw.bonePos[player,ii].z - _bones[ii].transform.localPosition.z/scale)*(sw.bonePos[player,ii].z - _bones[ii].transform.localPosition.z/scale);
					//if(delta > this.epsilon){
						...
						//this.positionLogFile.WriteLine("xxx");
					//}
				}
			}
		}
